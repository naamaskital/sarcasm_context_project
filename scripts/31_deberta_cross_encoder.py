...
REPORT_DIR = ROOT / "reports" / "full_dataset" / "deberta_cross_encoder"
MODEL_DIR = ROOT / "models" / "deberta_cross_encoder"
...
def require_cuda():
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for full-data DeBERTa fine-tuning.")
...
def make_dataset(df, tokenizer, mode):
    ...
    if mode == "comment_only":
        ds = Dataset.from_dict({"comment": df["comment"].astype(str).tolist(), "label": labels})
        def tokenize(batch):
            return tokenizer(batch["comment"], truncation=True, max_length=MAX_LENGTH, padding="max_length")
        return ds.map(tokenize, batched=True, remove_columns=["comment"])
    if mode == "context_only":
        ds = Dataset.from_dict({"context": df["context"].astype(str).tolist(), "label": labels})
        def tokenize(batch):
            return tokenizer(batch["context"], truncation=True, max_length=MAX_LENGTH, padding="max_length")
        return ds.map(tokenize, batched=True, remove_columns=["context"])
    if mode == "context_plus_comment":
        ...
        def tokenize(batch):
            return tokenizer(
                batch["context"], batch["comment"],
                truncation=True, max_length=MAX_LENGTH, padding="max_length",
            )
        return ds.map(tokenize, batched=True, remove_columns=["context", "comment"])
...
def train_mode(train_df, val_df, test_df, mode, resume=False, skip_completed=False):
    ...
    args = TrainingArguments(
        ...
        dataloader_pin_memory=True, **precision_flags(),
    )
    ...
    result = {
        "model": "deberta-v3-base cross-encoder", "input": mode, "epochs": NUM_EPOCHS,
...
def main():
    ...
    summary.to_csv(REPORT_DIR / "deberta_cross_encoder_metrics.csv", index=False)
    print("\nFINAL DEBERTA")
