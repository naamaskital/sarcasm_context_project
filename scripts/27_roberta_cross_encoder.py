...
REPORT_DIR = ROOT / "reports" / "full_dataset" / "roberta_cross_encoder"
MODEL_DIR = ROOT / "models" / "roberta_cross_encoder"
...
def require_cuda():
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for full-data RoBERTa fine-tuning.")
...
def make_dataset(df, tokenizer, mode):
    ...
    if mode == "comment_only":
        ds = Dataset.from_dict({"text": df["comment"].astype(str).tolist(), "label": labels})
    ...
    if mode == "context_only":
        ds = Dataset.from_dict({"text": df["context"].astype(str).tolist(), "label": labels})
...
def train_mode(train_df, val_df, test_df, mode, resume=False, skip_completed=False):
    ...
    pred_out["prediction"] = test_preds
    pred_out["prob_sarcastic"] = test_probs
    pred_out.to_parquet(REPORT_DIR / f"{mode}_test_predictions.parquet", index=False)
    ...
    result = {
        ...
        "validation_sarcastic_f1": val_metrics["sarcastic_f1"],
...
def main():
    ...
    summary.to_csv(REPORT_DIR / "roberta_cross_encoder_metrics.csv", index=False)
