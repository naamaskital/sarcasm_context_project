...
REPORT_DIR = ROOT / "reports" / "full_dataset" / "bert_cross_encoder"
MODEL_DIR = ROOT / "models" / "bert_cross_encoder"
...
def require_cuda():
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for full-data BERT fine-tuning.")
...
def train_mode(train_df, val_df, test_df, mode, resume=False, skip_completed=False):
    ...
    result = {
        "model": "bert-base-uncased cross-encoder",
        "input": mode,
        "epochs": NUM_EPOCHS,
...
def main():
    ...
    summary.to_csv(REPORT_DIR / "bert_cross_encoder_metrics.csv", index=False)
