...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Mandatory basic controls requested for the project: Qwen model-size comparison "
            "and prompt-formatting ablation on a fixed balanced subset of the full test split."
        )
    )
...
def format_example(row, input_mode, prompt_format, include_label=True):
    if input_mode == "comment_only":
        body = f"Reply:\n{row['comment']}"
    elif input_mode == "context_plus_comment":
...
def build_messages(row, few_shot_df, input_mode, prompt_format):
    ...
    for _, demo in few_shot_df.iterrows():
        demos.append(format_example(demo, input_mode, prompt_format, include_label=True))
    ...
    return [
        {
            "role": "system",
            "content": "You are a strict binary sarcasm classifier. Answer only with 0 or 1.",
        },
...
def make_balanced_sample(df, examples_per_class, seed):
    ...
    if any(counts.get(label, 0) < examples_per_class for label in [0, 1]):
        raise ValueError(
            f"Not enough examples for {examples_per_class} per class. Counts: {counts.to_dict()}"
        )
    return (
...
def metrics(y, pred):
    ...
    # Invalid generations count as incorrect for this all-example accuracy.
    correct_all = valid & (pred == y)
    accuracy_all = float(correct_all.mean())
...
def experiment_settings(model_id):
    settings = [
        ("comment_only", "structured", "size_comparison"),
        ("context_plus_comment", "structured", "size_comparison"),
    ]
...
def main():
    ...
    rows = []
    prediction_table = eval_df[["context", "comment", "label"]].copy()
    ...
    with open(REPORT_DIR / "qwen_basic_controls_summary.txt", "w", encoding="utf-8") as f:
        f.write("Qwen basic controls\n")
        f.write("=" * 70 + "\n")
        f.write(f"Balanced evaluation subset: {len(eval_df)} examples\n")
        f.write(f"Examples per class: {args.examples_per_class}\n")
        f.write(f"Few-shot demonstrations per class: {FEW_SHOT_PER_CLASS}\n\n")
        f.write("Size comparison: Qwen2.5-0.5B vs Qwen2.5-1.5B under identical few-shot prompts.\n")
        f.write("Prompt-format ablation: structured Context/Reply fields vs plain concatenation for Qwen2.5-0.5B.\n\n")
